    ...
    async addWatermark() {
        ...
        return sharp(getWatermarkPath(project)).ensureAlpha(getWatermarkOpacity(project)).toBuffer().then((value) => {
            return file.composite([{ input: value, gravity: 'southeast', blend: 'add' }]).toBuffer({ resolveWithObject: true }).then((value => {
                ...
                // FIXME workaround before refactoring uploadImagesToGCS
                .then(async (data) => {
                    await sharp(data).toFile(`./public/images/${this.newFilename}`, (err, info) => {
                        if (!!err) {
                            console.error(err)
                            throw err
                        }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                        ...
                        if (width < targetWidth) {
                            file.clone().toBuffer({ resolveWithObject: true }).then((value) => {
                                ...
                                sharp(value.data).toFile(`./public/images/${resized_filename}`).then(info => {
                                    resolve(`./public/images/${resized_filename}`)
                                }).catch(reason => {
                                    console.error(reason)
                                    throw reason
                                })
                ...
                await Promise.all(resizeJobs).finally(() => {
					...
					console.log("keep the files to debug")
