...
class ImageAdapter extends GCSAdapter {
    ...
    async addWatermark() {
        ...
        return sharp(getWatermarkPath(project)).ensureAlpha(getWatermarkOpacity(project)).toBuffer().then((value) => {
            return file.composite([{ input: value, gravity: 'southeast', blend: 'add' }]).toBuffer({ resolveWithObject: true }).then((value => {
                this.image.original = value
                return value.data
            })).catch(reason => {
                console.error(reason)
    ...
    async sync_save() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                    ...
                    let count = resizeJobs.push(new Promise((resolve) => {
                        ...
                        if (width < targetWidth) {
                            file.clone().toBuffer({ resolveWithObject: true }).then((value) => {
                                ...
                                feedDimensionToApiData(resizeKey, {
                                    width: width,
                                    height: height,
                                }, this.apiData)
                            ...
                            file.clone().resize(targetWidth, targetHeight, {
                                fit: 'inside',
                            }).toBuffer({ resolveWithObject: true }).then((value) => {
                                this.image[resizeKey] = value
                                feedDimensionToApiData(resizeKey, {
                                    width: targetWidth,
                                    height: targetHeight,
                ...
                await Promise.all(resizeJobs).catch((err) => { throw `error in resizing: ${err}` })
                ...
                for (const resizeKey in resizeTarget) {
                    ...
                    uploadPromises.push(uploadBufferToGCS(resized_filename, this.image[resizeKey].data, this.bucket))
                }
                await Promise.all(uploadPromises).catch(reason => reject(reason)).finally(() => {
                ...
                }).catch((err) => { throw `error in uploading to gcs: ${err}` })
